...
--shadow-search-bar: 0 4px 4px 0 rgba(0, 0, 0, 0.05);
```
...
## UI Blocks / Готовые блоки интерфейса

Готовые композитные блоки, которые можно использовать и расширять в зависимости от требований проекта.

### CardsLight - Карточка места/отеля (светлая тема)

**Назначение:**
Карточка для отображения мест, отелей, объектов недвижимости с фотографией, рейтингом, отзывами и ценой.

**Где использовать:**
- Туристические приложения (booking, airbnb-подобные)
- Каталоги недвижимости
- Списки мест/объектов с рейтингом
- E-commerce для аренды/услуг

**Компоненты:**
- Изображение с градиентным overlay
- Бейджи на изображении (Popular, Great view и т.д.)
- Метка "Sponsored"
- Рейтинг с цветным индикатором (зеленый для отличного)
- Название и локация
- Количество отзывов
- Кнопка с ценой

**Используемые токены:**
- **Colors**: `--color-yellow-20`, `--color-gray-8`, `--color-gray-2`, `--color-black`, `--color-gray-5`, `--color-success`, `--color-gray-6`, `--color-yellow`
- **Typography**: 11px (2xs), 15px (sm-plus), 10px, 16px (base), 14px (sm), 13px (xs-plus), 12px (xs), 18px (md)
- **Border Radius**: 30px (3xl), 15px (xl-2), 10px (2.5), 9px (lg-2), 5px (base-plus)
- **Spacing**: 30px (7.5), 10px (2.5), 5px (1.25), 3px (0.75), 16px (4), 6px (1.5)
- **Gradients**: `--gradient-overlay-dark`

**Гибкость:**
Блок можно легко расширять: добавлять больше бейджей, дополнительные метки, изменять количество информационных полей, добавлять кнопки действий (избранное, поделиться и т.д.).

**Связь с другими блоками:**
- Базовый блок для каталогов, используется в `BookingLight`

---

### BookingLight - Страница поиска и бронирования (светлая тема)

**Назначение:**
Полноценная страница/экран для поиска и просмотра результатов с панелью фильтрации по локации и датам. Комбинирует поисковую панель с карточкой места.

**Где использовать:**
- Туристические приложения с поиском по датам (Booking.com, Airbnb)
- Приложения для бронирования отелей/апартаментов
- Каталоги с временными фильтрами (аренда авто, поездки)
- Любые сервисы с поиском по локации + даты

**Компоненты:**
- Status bar (iOS/Android совместимость)
- Панель поиска с тенью:
  - Кнопка "Назад"
  - Поле с локацией и датами (компактный формат)
  - Кнопка фильтров/настроек
- Карточка места (вариант без яркой кнопки цены):
  - Изображение с темными бейджами (Popular, Great view)
  - Метки статуса (New, Awesome rating)
  - Полное название и детальная локация
  - Рейтинг и отзывы
  - Цена в текстовом формате "/ month"

**Используемые токены:**
- **Colors**: `--color-gray-9`, `--color-black`, `--color-black-30`, `--color-black-20`, `--color-gray-8`, `--color-gray-2`, `--color-gray-5`, `--color-success`, `--color-gray-6`, `white`
- **Typography**: 12px (xs), 11px (2xs), 16px (base), 13px (xs-plus), 14px (sm)
- **Border Radius**: 30px (3xl), 12px (xl), 15px (xl-2), 10px (2.5), 9px (lg-2), 5px (base-plus)
- **Spacing**: 10px (2.5), 16px (4), 7px (1.75), 15px (3.75), 5px (1.25), 3px (0.75)
- **Shadows**: `--shadow-search-bar` (для панели поиска)
- **Gradients**: Нет (бейджи с полупрозрачным фоном вместо градиента)

**Отличия от CardsLight:**
- Добавлена панель поиска с тенью
- Бейджи на карточке имеют темный полупрозрачный фон (`black-20`) вместо желтого
- Цена показана текстом без яркой желтой кнопки
- Используются метки "New" и "Awesome rating" вместо "Sponsored"

**Гибкость:**
Блок легко адаптируется:
- Панель поиска: можно добавлять больше фильтров (гости, тип жилья и т.д.)
- Карточка: расширяется как CardsLight - больше бейджей, дополнительная информация
- Формат даты: легко меняется на другие форматы
- Можно добавить список карточек вместо одной

**Связь с другими блоками:**
- Использует карточку из `CardsLight` с небольшими модификациями
- Совместима с `SocialLight` (похожая панель поиска)
